#!/bin/bash
workdir=./
sexconfpath=/products/sextractor_conf/
...
sex $filename -c ${sexconfpath}/fwhm.sex -PARAMETERS_NAME ${sexconfpath}/fwhm.par -FILTER_NAME ${sexconfpath}/gauss_2.0_5x5.conv -STARNNW_NAME ${sexconfpath}/default.nnw -VERBOSE_TYPE QUIET

fwhm=`awk '{print $1}' $tmp_fwhm_file | sort | awk ' { a[i++]=$1; } END { x=int((i+1)/2); if (x < (i+1)/2) print (a[x-1]+a[x])/2; else print a[x-1]; }'`
#echo "FWHM is: $fwhm for image $filename"
echo $fwhm > $fwhmfile
