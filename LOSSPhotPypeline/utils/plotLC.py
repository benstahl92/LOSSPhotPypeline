...
class plotLC:
    ...
    def load_lm(self, lm_file):
        '''loads "raw" light curves generated by LOSSPhotPypeline.pipeline.LPP'''
        df = pd.read_csv(lm_file, delim_whitespace = True, comment = ';',
                         names = ('mjd', 'etburst', 'mag', '-emag', '+emag', 'limmag', 'filter', 'imagename'))
        self.lm = pd.concat([df[df['filter'].str.upper() == filt][['mjd', 'limmag']].set_axis(['MJD', filt], axis = 'columns', inplace = False)
                             for filt in self.filter_ref], sort = False)
